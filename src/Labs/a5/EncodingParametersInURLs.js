...
function EncodingParametersInURLs() {
    ...
    const fetchSum = async (a, b) => {
        const response = await
            axios.get(`${LAB_BASE}/a5/add/${a}/${b}`);
        setResult(response.data);
    };
    const fetchSubtraction = async (a, b) => {
        const response = await axios.get(
            `${LAB_BASE}/a5/subtract/${a}/${b}`);
        setResult(response.data);
    };
    useEffect(() => {
        fetchSum();
    ...
    return (
        <div>
            ...
            <a
                href={`${LAB_BASE}/a5/add/${a}/${b}`}
                className="btn btn-primary">
                Add {a} + {b}
            </a>
            <a
                href={`${LAB_BASE}/a5/subtract/${a}/${b}`}
                className="btn btn-danger">
                Substract {a} - {b}
            </a>
            <h3>Query Parameters</h3>
            <a
                href={`${LAB_BASE}/a5/calculator?operation=add&a=${a}&b=${b}`}
                className="btn btn-primary">
                Add {a} + {b}
            </a>
            <a
                href={`${LAB_BASE}/a5/calculator?operation=subtract&a=${a}&b=${b}`}
                className="btn btn-danger">
                Substract {a} - {b}
            </a>
        </div>
